...
const ThreeBackground: React.FC<ThreeBackgroundProps> = ({
  ...
  const [currentGeometry, setCurrentGeometry] =
    useState<GeometryType>(geometryType)
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.FogExp2(isLightMode ? 0xf0f4f8 : 0x050505, 0.002)
    ...
    const animate = () => {
      ...
      if (cameraRef.current) {
        ...
        cameraRef.current.position.x +=
          (x * 0.005 - cameraRef.current.position.x) * 0.05
        cameraRef.current.position.y +=
          (-y * 0.005 - cameraRef.current.position.y) * 0.05
