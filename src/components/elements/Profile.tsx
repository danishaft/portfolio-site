...
const Profile = (props: ProfileProps): React.ReactElement => {
  ...
  const avatarElement = profile.avatar ? (
    <div className="flex-shrink-0">
      <Avatar avatar={profile.avatar} className="w-72 h-72 sm:w-80 sm:h-80 lg:w-96 lg:h-96 rounded-full overflow-hidden shadow-2xl" />
    </div>
  ) : null;
  ...
  const tagsElement = profile?.tags && profile.tags.length > 0 ? (
    <div className="mb-6">
    ...
    <div>
      ...
      <div className="flex flex-col-reverse items-center lg:flex-row lg:justify-between lg:items-center gap-8 lg:gap-12">
        ...
        <div className="flex flex-col items-center lg:items-start max-w-xl">
          ...
          <H level={hLevel.h1} className="font-display text-5xl sm:text-6xl lg:text-7xl font-bold uppercase tracking-tight mb-4 text-center lg:text-left">
            {profile?.firstName}
            <br />
            {profile?.lastName}
          </H>
