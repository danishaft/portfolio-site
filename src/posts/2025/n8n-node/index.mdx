...
- **Auto-Map Input Data to Columns** → n8n maps the incoming JSON object directly to table columns. Perfect if the JSON keys from the previous node match the column names in the Neon database
- **Manual Map (Define Values Below)** → if keys don't match, the user will pick each column and set its value themselves. More tedious, but gives full control.
...
Looks fine, but here's the catch—**n8n expressions (`{{ $json.userId }}`) aren't plain strings**. They're dynamic placeholders. I passed them straight into my query, and Neon kept throwing errors.
...
1. **Select Rows** → A fixed collection where users pick a column, an operator (`=`, `>`, `<`, etc.), and a value. Example: _Column = age, Operator = >, Value = 18._
2. **Combine Conditions** → A simple dropdown: `AND` or `OR`. This lets users build compound filters.
3. **Sort** → A collection field where users choose a column and direction (`ASC` or `DESC`).
...
- User config → `{ column: 'age', operator: '>', value: 18 }`
- Values → `['age', 18]`
